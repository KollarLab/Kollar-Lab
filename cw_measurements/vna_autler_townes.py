...
def vna_autler_townes(instruments, settings):
    ...
    settings['RFpower']      = exp_settings['RFpower']      + Qbit_Attenuation
    settings['autler_power'] = exp_settings['autler_power'] + Autler_Attenuation
    
    autlergen.power = exp_settings['autler_power']
    autlergen.output = 'On'
    SRS.output = 'On'
    ...
    for freqind in findices:
        ...
        if freqind==0:
            tstop = time.time()
            estimate_time(tstart, tstop, len(autler_freq))

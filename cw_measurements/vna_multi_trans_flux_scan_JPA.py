...
def vna_multi_trans_flux_scan_JPA(instruments, settings):
    '''
    ...
    filename_template = exp_settings['scanname'] + '_Vbias_{}V_' + stamp
    ...
    pump_powers = np.round(np.linspace(start_pump_power - pump_amp - line_attenuation, stop_pump_power - pump_amp - line_attenuation, power_points),6)
    max_power = 13
    min_power = -25
    ...
    for vind in range(len(voltages)):
        mags = np.zeros((power_points, pump_freq_points))
        phases = np.zeros((power_points, pump_freq_points))
        ...
        stamp    = userfuncs.timestamp()
        filename = filename_template.format(voltage)
        ...
        for pind in range(len(pump_powers)):
            ...
            for find in range(len(pump_freqs)):
                pump_gen.set_frequency(pump_freqs[find])
                ...
                if exp_settings['subtract_Pump'] == 'On':
                    ...
                    mags[pind,find]   = data['mag'][center_index] - data_off['mag'][center_index]
                    phases[pind,find] = data['phase'][center_index] - data_off['phase'][center_index]
                else:
                    mags[pind,find]   = np.mean(data['mag'])
                    phases[pind,find] = np.mean(data['phase'])
            ...
            labels = ['Pump Freq (GHz)', 'Pump Power (dBm)']

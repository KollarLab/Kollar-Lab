...
def get_default_settings():
    ...
    #Save location
    settings['scanname']    = 'test_0'
    settings['meas_type']   = 'trans_power_and_phi_scan'
    ...
    settings['stop_power'] = -49
    settings['power_points'] = 2

    return settings

def vna_power_and_phi_scan(instruments, settings, avg_span, detection_span):
    ##Instruments used
    vna = instruments['VNA']
    ...
    phis = np.round(np.linspace(start_phi, stop_phi, phi_points), 2)
    ...
    center_higher_magnitude = np.zeros((power_points, phi_points))-60
    ...
    # start measurement
    t0 = time.time()
    for pind, power in enumerate(powers):
        ...
        t1 = time.time() #to account for the time taken by the calibration traces in each power

        # Now we sweep phi within each power
        for phi_ind in range(len(phis)):
            ...
            if phi_ind==0 and pind == 0:
                ...
                avg_time_per_power = avg_interval_1 + avg_interval_2*len(phis)
                avg_total_time = np.round((avg_time_per_power*len(powers))/3600, 1) # in hrs
            ...
            if S41_mag_sliced[int((len(S41_mag_sliced)-1)/2)] > S23_mag_sliced[int((len(S23_mag_sliced)-1)/2)]:
                ...
                center_isolation_contrast_calibrated[pind, phi_ind] = center_isolation_contrast_raw[pind, phi_ind] - transmission_mag_offset[0]
                center_higher_magnitude[pind, phi_ind] = S41_mag_sliced[index_S23_min]
                ...
                center_isolation_contrast_calibrated[pind, phi_ind] = center_isolation_contrast_raw[pind, phi_ind] - transmission_mag_offset[0]
                center_higher_magnitude[pind, phi_ind] = S23_mag_sliced[index_S41_min]
            ...
            full_data['center_isolation_contrast_raw'] = center_isolation_contrast_raw[pind]
            full_data['center_isolation_contrast_calibrated'] = center_isolation_contrast_calibrated[pind]
            full_data['transmission_mag_offset'] = transmission_mag_offset[0]



            fig = plt.figure(56, figsize=(12,9))
            plt.clf()
            ...
            plt.xlabel('Frequency (GHz)')
            plt.ylabel('Mag (dB)')
            plt.title('Live Traces')
            plt.legend(loc='lower right')

            # ax = plt.subplot(2,2,4)
            # plt.imshow(center_isolation_contrast_calibrated, extent=[phis[0], phis[-1], powers[0], powers[-1]], origin='lower', aspect='auto', cmap='RdBu_r')
            # plt.colorbar(label='Center Isolation Contrast (dB)')
            # plt.xlabel('phi (degree)')
            # plt.ylabel('fm (MHz)')
            # plt.title('Power and phi Sweep')

            ax = plt.subplot(2,2,4)

            for i in range(len(phis)):
                plt.plot(powers-CAV_Attenuation, center_higher_magnitude[:, i], label=str(phis[i]))
            plt.xlabel('Input Power (dB)')
            plt.ylabel('Magnitude (dB)')
            plt.title('Power and phi Scan')
    ...
    t3 = time.time()
    print('Elapsed time: {} hrs'.format(np.round((t3-t0)/3600, 2)))

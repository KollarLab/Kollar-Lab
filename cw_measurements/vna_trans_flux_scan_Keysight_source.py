...
def vna_trans_flux_scan(instruments, settings, channel=1):
    ##Instruments used
    vna = instruments['VNA']
    ...
    if channel == 1:
        ...
        print('Flux voltage output through channel 1')

    elif channel == 2:
        Dual_gen.Ch2_dc_voltage_ramp(0)
        Dual_gen.Ch2_output = 1
        print('Flux voltage output through channel 2')
    ...
    for pind in range(len(powers)):
        ...
        for vind in range(len(voltages)):
            ...
            if channel == 1:
                Dual_gen.Ch1_dc_voltage_ramp(voltage)
                time.sleep(0.1)
                t0 = time.time()
            
            elif channel == 2:
                Dual_gen.Ch2_dc_voltage_ramp(voltage)
                time.sleep(0.1)
                t0 = time.time()

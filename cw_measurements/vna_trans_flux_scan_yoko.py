...
def vna_trans_flux_scan(instruments, settings):
    ...
    vna.reset()
    vna.inst.write('SENS1:CORR:EDEL1:ELEN 25.875')
    ...
    # phase_init_unwrap = np.unwrap(data_init['phase'], period = 360)
    ...
    for pind in range(len(powers)):
        ...
        for vind in range(len(currents)):
            ...
            mags[vind]   = data['mag']
            # phases[vind] = data['phase'] - phase_init
            phases[vind] = np.unwrap(data['phase'], period=360) #- phase_init_unwrap

...
def phase_freq(x, a, b):
    return a*x + b

def vna_trans_flux_scan(instruments, settings):
    ##Instruments used
    vna = instruments['VNA']
    yoko = instruments['current_supply']

    vna.reset()
    vna.inst.write('SENS1:CORR:EDEL1:ELEN 25.835')
    ...
    jpa_response = np.zeros((current_points, exp_settings['freq_points']))
    ...
    for pind in range(len(powers)):
        ...
        for vind in range(len(currents)):
            ...
            # get initial data
            # yoko.Output = 'Off'
            # data_init = vna.trans_meas(exp_settings)
            # phase_init = data_init['phase']
            # yoko.Output = 'On'
            # ###############
           ...
           # phases[vind] = np.unwrap(data['phase'], period=360)

            # get the reference phase curve from the first row
            popt, pcov = curve_fit(phase_freq, freqs, phases[0], p0=[1, 1], method='lm')

            #jpa_response[vind] = phases[vind] / phase_freq(freqs, popt[0], popt[1])
            jpa_response[vind] = phases[vind] #- phases[0] #_init
            ...
            full_data['phases'] = jpa_response[0:vind+1]

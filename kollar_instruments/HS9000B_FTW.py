...
'''
rounding the Holzworth input frequency into FTW values such that it matches a givien trigger rate trigger rate in MHz

'''

def nearest_FTW(freq,band = 6):
    '''
    Parameters
    ----------
    freq : MHz note: it only works in the 4-8GHz range
        DESCRIPTION.
    band : Int, optional
    ...
    Returns
    -------
    nearest_FTW : round to the nearst FTW word
        DESCRIPTION.

    '''
    FTW = (freq * 2**48)/(10**9*2**band)
    if FTW%2 < 1:
        nearest_FTW = np.floor(FTW)
    elif FTW%2 == 1:
        nearest_FTW = FTW + 1
    else:
        nearest_FTW = np.ceil(FTW)
        
    return nearest_FTW

def FTW_to_freq(FTW,band = 6):
    freq = (10**9*2**band*FTW)/2**48
    return freq
...
class HS9000B():
    ...
    '''
    def __init__(self, address,trig):
        '''
        ...
        self._trigger = trig
        self.ch1 = channel(self.inst, 1, self._trigger)
        self.ch2 = channel(self.inst, 2, self._trigger)
    ...
    def settings(self):
        ...
        fullsettings['trig'] = self._trigger
...
class channel(object):
    ...
    '''
    def __init__(self, inst, number,trigger):
...
#            fullsettings[setting] = self.__getattr__(setting)
    ...
    def freq(self):
        ...
        writeStr = channel + ':FREQ:FTW?'
        output = self.inst.query(writeStr)
        # example '1980.00000 MHz'
        # val = float(output[0:-3])*1e6
        val = FTW_to_freq(float(output),band = 6)
        return val
    @freq.setter
    def freq(self, val):
        '''value should be a number in FTW 
        This is currently hard coded for band=6 (4-8 GHz)
        '''
        #val_MHz = val/1e6
        #val_MHz = np.round(val_MHz,9)
        # print(val)
        set_freq = val
        set_FTW = nearest_FTW(set_freq, 6)
        trigger_rate = self._trig
        set_trigger_FTW = nearest_FTW(trigger_rate, 6)
        set_FTW = np.round(set_FTW/set_trigger_FTW)*set_trigger_FTW
        set_freq = FTW_to_freq(set_FTW,band = 6)
        channel = self.__dict__['channel']
        val_MHz = np.round(set_freq/1e6,9)
        writeStr_freq = channel + ':FREQ:' + str(val_MHz) + 'MHz'
        writeStr_FTW = channel + ':FREQ:FTW:' + str(int(set_FTW))
        # print(writeStr_freq)
        # print(writeStr_FTW)
        self.inst.query(writeStr_freq)

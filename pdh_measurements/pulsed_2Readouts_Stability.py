...
def pulsed_2Readouts_Stability(instruments, settings):
    ##Instruments used
    ...
    for numind in range(num_points):
        ...
        for find in range(0, len(freqs)):
            ...
            if exp_globals['IF'] != 0:
                ...
                new_IF = freq - LO.freq
                # carrier
                digLO_sin_carr = np.sin(2*np.pi*new_IF*xaxis)
                digLO_cos_carr = np.cos(2*np.pi*new_IF*xaxis)
                settings['digLO_sin_carr'] = digLO_sin_carr 
                settings['digLO_cos_carr'] = digLO_cos_carr
                
                # lower sideband
                digLO_sin_sb = np.sin(2*np.pi*(-1)*(mod_freq-new_IF)*xaxis )
                digLO_cos_sb = np.cos(2*np.pi*(1)*(mod_freq-new_IF)*xaxis )
                settings['digLO_sin_sb'] =  digLO_sin_sb 
                settings['digLO_cos_sb'] = digLO_cos_sb 

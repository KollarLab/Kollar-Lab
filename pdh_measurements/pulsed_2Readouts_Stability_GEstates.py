...
def pulsed_2Readouts_Stability_GEstates(instruments, settings):
    ##Instruments used
    ...
    I_carr_g, Q_carr_g = np.zeros((num_points, len(freqs))), np.zeros((num_points, len(freqs)))
    powerdat_carr_g, phasedat_carr_g = np.zeros((num_points, len(freqs))), np.zeros((num_points, len(freqs)))
    I_sb_g, Q_sb_g = np.zeros((num_points, len(freqs))), np.zeros((num_points, len(freqs)))
    powerdat_sb_g, phasedat_sb_g = np.zeros((num_points, len(freqs))), np.zeros((num_points, len(freqs)))
    phasedat_diff_g = np.zeros((num_points, len(freqs)))
    
    I_carr_e, Q_carr_e = np.zeros((num_points, len(freqs))), np.zeros((num_points, len(freqs)))
    powerdat_carr_e, phasedat_carr_e = np.zeros((num_points, len(freqs))), np.zeros((num_points, len(freqs)))
    I_sb_e, Q_sb_e = np.zeros((num_points, len(freqs))), np.zeros((num_points, len(freqs)))
    powerdat_sb_e, phasedat_sb_e = np.zeros((num_points, len(freqs))), np.zeros((num_points, len(freqs)))
    phasedat_diff_e = np.zeros((num_points, len(freqs)))
    ...
    for numind in range(num_points):
        ...
        print(f'Scan run:{numind+1}, CAV_power:{CAV_power - CAV_Attenuation}, Qbit_power:{Qbit_power - Qbit_Attenuation}')
        ...
        for find in range(0, len(freqs)):
            ...
            LO.output = 'On'
            qubitgen.output = 'Off'
            carr_data_g, sb_data_g, xaxis_g = read_and_process_2Readouts(card, settings, 
                                                                         plot=first_it, 
                                                                         IQstorage = True)
            ...
            if exp_settings['subtract_background']:
                ...
                carr_data_b = {key: 0 for key in carr_data_g}
                sb_data_b = {key: 0 for key in sb_data_g}
            qubitgen.output = 'On'
            time.sleep(0.1)
            carr_data_e, sb_data_e, xaxis_e = read_and_process_2Readouts(card, settings, 
                                                                         plot=first_it, 
                                                                         IQstorage = True)
            ...
            if exp_settings['subtract_background']:
                ...
                carr_data_b = {key: 0 for key in carr_data_e}
                sb_data_b = {key: 0 for key in sb_data_e}
                           
            I_final_carr_g = np.mean(carr_data_g['I_window'])  - np.mean(carr_data_b['I_window']) #compute <I> in the data window
            Q_final_carr_g = np.mean(carr_data_g['Q_window'])  - np.mean(carr_data_b['Q_window']) #compute <Q> in the data window
            
            I_final_carr_e = np.mean(carr_data_e['I_window'])  - np.mean(carr_data_b['I_window']) #compute <I> in the data window
            Q_final_carr_e = np.mean(carr_data_e['Q_window'])  - np.mean(carr_data_b['Q_window']) #compute <Q> in the data window
            
            I_final_sb_g = np.mean(sb_data_g['I_window']) - np.mean(sb_data_b['I_window']) #compute <I> in the data window
            Q_final_sb_g = np.mean(sb_data_g['Q_window']) - np.mean(sb_data_b['Q_window']) #compute <Q> in the data window
            
            I_final_sb_e = np.mean(sb_data_e['I_window']) - np.mean(sb_data_b['I_window']) #compute <I> in the data window
            Q_final_sb_e = np.mean(sb_data_e['Q_window']) - np.mean(sb_data_b['Q_window']) #compute <Q> in the data window
            ...
            Is_carr_g[find,:], Qs_carr_g[find,:] = carr_data_g['I_full'], carr_data_g['Q_full']
            Is_sb_g[find,:], Qs_sb_g[find,:] = sb_data_g['I_full'], sb_data_g['Q_full']
            
            Is_carr_e[find,:], Qs_carr_e[find,:] = carr_data_e['I_full'], carr_data_e['Q_full']
            Is_sb_e[find,:], Qs_sb_e[find,:] = sb_data_e['I_full'], sb_data_e['Q_full']
            
            Is_carr_b[find,:], Qs_carr_b[find,:] = carr_data_b['I_full'], carr_data_b['Q_full']
            Is_sb_b[find,:], Qs_sb_b[find,:] = sb_data_b['I_full'], sb_data_b['Q_full']
            
            I_carr_g[numind, find], Q_carr_g[numind, find] = I_final_carr_g, Q_final_carr_g
            powerdat_carr_g[numind, find], phasedat_carr_g[numind, find] = np.sqrt(I_final_carr_g**2 + Q_final_carr_g**2), np.arctan2(Q_final_carr_g, I_final_carr_g)*180/np.pi
            
            I_carr_e[numind, find], Q_carr_e[numind, find] = I_final_carr_e, Q_final_carr_e
            powerdat_carr_e[numind, find], phasedat_carr_e[numind, find] = np.sqrt(I_final_carr_e**2 + Q_final_carr_e**2), np.arctan2(Q_final_carr_e, I_final_carr_e)*180/np.pi
            
            
            I_sb_g[numind, find], Q_sb_g[numind, find] = I_final_sb_g, Q_final_sb_g
            powerdat_sb_g[numind, find], phasedat_sb_g[numind, find] = np.sqrt(I_final_sb_g**2 + Q_final_sb_g**2), np.arctan2(Q_final_sb_g, I_final_sb_g)*180/np.pi
            
            I_sb_e[numind, find], Q_sb_e[numind, find] = I_final_sb_e, Q_final_sb_e
            powerdat_sb_e[numind, find], phasedat_sb_e[numind, find] = np.sqrt(I_final_sb_e**2 + Q_final_sb_e**2), np.arctan2(Q_final_sb_e, I_final_sb_e)*180/np.pi
            
            
            phasedat_diff_g[numind, find] = phasedat_carr_g[numind, find] - phasedat_sb_g[numind, find]
            phasedat_diff_e[numind, find] = phasedat_carr_e[numind, find] - phasedat_sb_e[numind, find]
        ...
        if num_points > 1:
            ...
            fig.suptitle(f'{num_points} points phase stability | avgs:{avgs}')
            fig.tight_layout()
            plt.show()
            ...
            fig.suptitle(f'{num_points} points IQ stability | avgs:{avgs}')
            fig.tight_layout()
            plt.show()
            ...
            identifier = 'Power: {}dBm'.format(CAV_power-CAV_Attenuation)
            ...
            plt.clf()
            ax = plt.subplot(2,2,1)
            plt.plot(full_time['xaxis'], carr_data_g['I_full'], label = 'I_full_g')
            plt.plot(full_time['xaxis'], carr_data_g['Q_full'], label = 'Q_full_g')
            plt.legend()
            plt.xlabel('Time (us)')
            plt.title('single carr trace')
            
            ax = plt.subplot(2,2,2)
            plt.plot(full_time['xaxis'], sb_data_g['I_full'], label = 'I_full_g')
            plt.plot(full_time['xaxis'], sb_data_g['Q_full'], label = 'Q_full_g')
            plt.legend()
            plt.xlabel('Time (us)')
            plt.title('single sb trace')
            
            ax = plt.subplot(2,2,3)
            plt.plot(full_time['xaxis'], carr_data_e['I_full'], label = 'I_full_e')
            plt.plot(full_time['xaxis'], carr_data_e['Q_full'], label = 'Q_full_e')
            plt.legend()
            plt.xlabel('Time (us)')
            plt.title('single carr trace')
            
            ax = plt.subplot(2,2,4)
            plt.plot(full_time['xaxis'], sb_data_e['I_full'], label = 'I_full_e')
            plt.plot(full_time['xaxis'], sb_data_e['Q_full'], label = 'Q_full_e')
            plt.legend()
            plt.xlabel('Time (us)')
            plt.title('single sb trace')

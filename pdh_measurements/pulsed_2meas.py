...
def get_default_settings():
    ...
    settings['SB_freq']          = 6.98e9

    settings['SB_delay']         = 2e-6
    settings['meas2_delay']      = 4e-6
...
def pulsed_2meas(instruments, settings):
    ...
    CAV_power = exp_settings['CAV_Power'] + CAV_Attenuation + EnableIQ_loss
    CAV_freq  = exp_settings['CAV_freq']
    
    ##SB settings
    SB_freq = exp_settings['SB_freq']
    
    Qbit_Attenuation = exp_globals['Qbit_Attenuation']
    SB_start_power  = exp_settings['SB_start_power'] + Qbit_Attenuation + EnableIQ_loss
    SB_stop_power   = exp_settings['SB_stop_power']  + Qbit_Attenuation + EnableIQ_loss
    ...
    qubitgen.freq = SB_freq
    qubitgen.power = SB_powers[0]
    ...
    for pind in range(SB_power_points):
        ...
        print('SB off,  Current power:{}'.format(SB_powers[pind]-Qbit_Attenuation-EnableIQ_loss))
        qubitgen.Output  = 'Off'
...
#                time.sleep(0.1)
    ...
    # SB(stimulation) pulse, generate using qubitgen instead at the qubitgen freq
    qubit_I.add_pulse('pdh_I', position=SB_position,
                        amp_list = [0,1,0], phase_list = [0,0,0],
                        mod_freq = 0,
                        time = stm_time)
    ...
    # cavity marker on for whole time
    cavity_marker.add_window(start_time, start_time+cav2_delay+2*window_time)
    qubit_marker.add_window(SB_position, SB_position+stm_time)
    ...
    for pind in range(SB_power_points):
        ...
        print('SB on,  Current power:{}'.format(SB_powers[pind]-Qbit_Attenuation-EnableIQ_loss))
        qubitgen.Output  = 'On'
        qubitgen.Freq   = SB_freq
        qubitgen.Power  = SB_powers[pind]
...
#                time.sleep(0.1)
    ...
    # Plot
    arb = 100
    title = 'Mod freq(MHz): ' + str(np.abs(CAV_freq-SB_freq)/1e6) 

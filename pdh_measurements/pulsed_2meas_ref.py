...
def pulsed_2meas(instruments, settings):
    ...
    ref_amp_list = np.zeros((1,3))
    CAV_amp_list = np.zeros((1,3))
    SB_amp_list  = np.zeros((SB_power_points, 3))

    ref_amp_list[0] = get_amp_comps([-100, -36, -100], exp_settings['SGS_power'])
    for i in range(1):
    ...
    ## Set full_data
    # ref: reference with 3 states / spec: SB on
    Iblob_ref = np.zeros((1, segments))
    Qblob_ref = np.zeros((1, segments))
    powerdat_ref = np.zeros((1, segments))
    phasedat_ref = np.zeros((1, segments))
    Iblob_ref2 = np.zeros((1, segments))
    Qblob_ref2 = np.zeros((1, segments))
    powerdat_ref2 = np.zeros((1, segments))
    phasedat_ref2 = np.zeros((1, segments))
    ...
    # Always have pi/2 pulse for the reference 
    ...
    cavity_I.add_pulse('pdh_I', position=SB_position,
                        amp_list = ref_amp_list[0], phase_list = [0,0,0],
                        mod_freq = 0,
                        time = window_time)
...
#                time.sleep(0.1)
    ...
    Iblob_ref[0] = I_final
    Qblob_ref[0] = Q_final
    powerdat_ref[0] = np.sqrt(I_final**2 + Q_final**2)
    phasedat_ref[0] = np.arctan2(Q_final, I_final)*180/np.pi

    Iblob_ref2[0] = I_final2
    Qblob_ref2[0] = Q_final2
    powerdat_ref2[0] = np.sqrt(I_final2**2 + Q_final2**2)
    phasedat_ref2[0] = np.arctan2(Q_final2, I_final2)*180/np.pi

    Iblob_ref_avg = np.mean(Iblob_ref, axis=1, keepdims=True)
    Qblob_ref_avg = np.mean(Qblob_ref, axis=1, keepdims=True)
    Iblob_ref_avg2 = np.mean(Iblob_ref2, axis=1, keepdims=True)
    Qblob_ref_avg2 = np.mean(Qblob_ref2, axis=1, keepdims=True)

        
    ##Packaging data for the plotting functions and saving 
    ref_data = {}
    ref_data['xaxis'] = SB_powers
    ref_data['mags'] = powerdat_spec
    ref_data['phases'] = phasedat_spec
    ref_data['mags2'] = powerdat_spec2
    ref_data['phases2'] = phasedat_spec2

    ref_data['Iblob'] = Iblob_ref
    ref_data['Qblob'] = Qblob_ref
    ref_data['center'] = [Iblob_ref_avg, Qblob_ref_avg]
    ref_data['Iblob2'] = Iblob_ref2
    ref_data['Qblob2'] = Qblob_ref2
    ref_data['center2'] = [Iblob_ref_avg2, Qblob_ref_avg2]
...
#                time.sleep(0.1)
    ...
    lines_wref = (SB_power_points)//4 + 1
    ...
    cmap = base_cmap.copy()
    cmap.set_under('white')  # color for 0 counts
    
    for i in range(SB_power_points):
        ax = fig.add_subplot(gs[i//4:i//4+1, i%4:i%4+1])
        h = ax.hist2d(Iblob_trans2[i], Qblob_trans2[i], bins=200, range = [[-max_nosb, max_nosb],[-max_nosb, max_nosb]], cmap=cmap, vmin=1)
    ...
    # Second, the SB with ref data
    fig = plt.figure(num=301, figsize=(14,12))
    fig.set_figwidth(14)
    plt.clf()
    
    gs = gridspec.GridSpec(nrows=lines_wref, ncols=4, left=0.05, right=0.95, hspace=0.15, wspace=0.2, top=0.9, bottom=0.1)
    ...
    ax = fig.add_subplot(gs[0//4:0//4+1, 0%4:0%4+1])
    h = ax.hist2d(Iblob_ref2[0], Qblob_ref2[0], bins=200, range = [[-max_sb, max_sb],[-max_sb, max_sb]], cmap=cmap, vmin=1)
    ...
    for i in range(SB_power_points):
        ax = fig.add_subplot(gs[(i+1)//4:(i+1)//4+1, (i+1)%4:(i+1)%4+1])
        h = ax.hist2d(Iblob_spec2[i], Qblob_spec2[i], bins=200, range = [[-max_sb, max_sb],[-max_sb, max_sb]], cmap=cmap, vmin=1)

...
def get_default_settings():
    ...
    settings['meas_type'] = 'pulsed_DPA_4wm_cal'
...
def pulsed_DPA_4wm_cal(instruments, settings):
    ...
    filename_template = exp_settings['scanname'] + '_{}MHz_{}V_{}dBm_' + stamp
    ...
    phases = np.linspace(start_phase, stop_phase, phase_points)
    scissors_phase = 2*phases[:,1] - (phases[:,0] + phases[:,2])
    ...
    for vind in range(voltage_points):
        ...
        print(f'Current voltage: {voltages[vind]} V.')
        ...
        for pind in range(pump_power_points):
            ...
            current_power = pump_powers[pind] - exp_globals['Qbit_Attenuation']
            print(f'Current power: {current_power} dBm.')

            filename = filename_template.format(exp_settings['detuning']/1e6, voltages[vind], pump_powers[pind] - exp_globals['Qbit_Attenuation'])

            I_full = np.zeros((phase_points, cav_freq_points))
            Q_full = np.zeros((phase_points, cav_freq_points))
            powerdat_full = np.zeros((phase_points, cav_freq_points))
            phasedat_full = np.zeros((phase_points, cav_freq_points))
            diff_full = np.zeros((phase_points, cav_freq_points))
            ...
            yaxis = [0]
            labels = ['Cav Freq (GHz)', 'Pump power (off)']
            ...
            for phind in range(phase_points):
                ...
                cavity_I.add_pulse(
                    type='pdh_I', position=cav_position,
                    amp_list = cav_amp_list, phase_list = phases[phind],
                    mod_freq = 10e6,
                    time = window_time
                )
                cavity_Q.add_pulse(
                ...
                # pump pulse
                pump_I.add_pulse(
                    type='pdh_I', position=pump_position,
                    amp_list = pump_amp_list[pind], phase_list = phases[phind],
                    mod_freq = detuning,
                    time = pump_window + window_time
                )
                pump_Q.add_pulse(
                    type='pdh_Q', position=pump_position,
                    amp_list = pump_amp_list[pind], phase_list = phases[phind],
                    mod_freq = detuning,
                    time = pump_window + window_time
                )
                ...
                for find in range(cav_freq_points):
                    ...
                    I_full[phind, find] = I_sig
                    Q_full[phind, find] = Q_sig
                    powerdat_full[phind, find] = V_to_dBm(np.sqrt(I_sig**2 + Q_sig**2))
                    # powerdat_full[phind, find] = np.sqrt(I_sig**2 + Q_sig**2)
                    phasedat_full[phind, find] = np.arctan2(Q_sig, I_sig)*180/np.pi

                    if first_it:
                        first_it = False
                
                diff_full[phind] = powerdat_full[phind] - powerdat_off
    

                ## Save Full data
                # Until here, shape: (phase_points, cav_freq_points)
                full_data_dummy = {}
                full_data_dummy['xaxis'] = cav_freqs/1e9
                full_data_dummy['scissors_phase'] = scissors_phase[0:phind+1]
                full_data_dummy['mags'] = diff_full[0:phind+1] 
                full_data_dummy['phases'] = phasedat_full[0:phind+1]
                full_data_dummy['abs_power'] = powerdat_full[0:phind+1]
                full_data_dummy['I'] = I_full[0:phind+1]
                full_data_dummy['Q'] = Q_full[0:phind+1]

                single_data = {}
                single_data['xaxis'] = cav_freqs/1e9
                single_data['mag'] = powerdat_full[phind, :]
                single_data['phase'] = phasedat_full[phind, :]

                yaxis = scissors_phase[0:phind+1]
                labels = ['Cav Freq (GHz)', 'Scissors Phase (mod pi)']
                simplescan_plot(full_data_dummy, single_data, yaxis, filename, labels, identifier='', fig_num=1)

            # change the shape (cav_freq_points, phase_points)
            full_data = {}
            full_data['xaxis'] = scissors_phase
            full_data['mags'] = diff_full.T
            full_data['phases'] = phasedat_full.T
            full_data['abs_power'] = powerdat_full.T
            full_data['I'] = I_full.T
            full_data['Q'] = Q_full.T
            ...
            yaxis = cav_freqs
            labels = ['Scissors Phase (pi)', 'Cav Freq (GHz)']
            simplescan_plot(full_data, single_data, yaxis, filename, labels, identifier='', fig_num=1)
            plt.savefig(os.path.join(saveDir, filename+'.png'), dpi = 150)
            
            userfuncs.SaveFull(saveDir, filename, ['full_data', 'full_data_off', 'voltages', 'detuning', 'current_voltage', 'current_power'], 

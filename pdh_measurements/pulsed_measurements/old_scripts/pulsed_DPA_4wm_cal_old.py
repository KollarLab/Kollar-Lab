...
def get_default_settings():
    ...
    settings['start_detuning']  = 10e6
    settings['stop_detuning']   = 20e6
    settings['detuning_points'] = 10
...
def pulsed_DPA_4wm_cal(instruments, settings):
    ...
    filename_template = exp_settings['scanname'] + '_{}V_{}MHz_' + stamp
    ...
    # Pump settings
    detuning_points = exp_settings['detuning_points']
    detuning_freqs = np.round(np.linspace(exp_settings['start_detuning'], exp_settings['stop_detuning'], detuning_points),-3)

    if np.any(detuning_freqs <= 0):
    ...
    for vind in range(voltage_points):
        ...
        for dind in range(detuning_points):
            first_it = True

            current_detuning = detuning_freqs[dind]
            print(f'Current detuning: {current_detuning/1e6} MHz.')

            filename = filename_template.format(voltages[vind], current_detuning/1e6)

            I_full = np.zeros((pump_power_points, cav_freq_points))
            Q_full = np.zeros((pump_power_points, cav_freq_points))
            powerdat_full = np.zeros((pump_power_points, cav_freq_points))
            phasedat_full = np.zeros((pump_power_points, cav_freq_points))
            diff_full = np.zeros((pump_power_points, cav_freq_points))
            ...
            for pind in range(pump_power_points):
                ...
                pump_I.add_pulse(
                    ...
                    amp_list = pump_amp_list[pind], phase_list = pump_phase_list,
                    mod_freq = detuning_freqs[dind],
                ...
                for find in range(cav_freq_points):
                    ...
                    I_full[pind, find] = I_sig
                    Q_full[pind, find] = Q_sig
                    powerdat_full[pind, find] = V_to_dBm(np.sqrt(I_sig**2 + Q_sig**2))
                    # powerdat_full[pfind, find] = np.sqrt(I_sig**2 + Q_sig**2)
                    phasedat_full[pind, find] = np.arctan2(Q_sig, I_sig)*180/np.pi
                ...
                full_data['pump_powers'] = pump_powers[0:pind+1] - exp_globals['Qbit_Attenuation']
                ...
                yaxis = pump_powers[0:pind+1] - exp_globals['Qbit_Attenuation']
                labels = ['Cav Freq (GHz)', 'Pump power (dBm)']
                simplescan_plot(full_data, single_data, yaxis, filename, labels, identifier='', fig_num=1)
                plt.savefig(os.path.join(saveDir, filename+'.png'), dpi = 150)
            
            userfuncs.SaveFull(saveDir, filename, ['full_data', 'full_data_off', 'voltages', 'detuning_freqs', 'current_voltage', 'current_detuning'], 

...
def get_default_settings():
    ...
    settings['meas_type'] = 'pulsed_NDPA_cal_detune'
...
def pulsed_NDPA_cal_detune(instruments, settings):
    ...
    filename_template = exp_settings['scanname'] + '_{}V_detune{}MHz_' + stamp
    ...
    # Pump settings; config: cav | pump | idler
    pump_freqs = cav_freqs + exp_settings['detuning']

    if np.abs(exp_globals['IF']) >= np.abs(exp_settings['detuning']):
        raise ValueError('IF should be smaller than detuning.')
    ...
    for vind in range(voltage_points):
        ...
        first_it = True
        filename = filename_template.format(voltages[vind], exp_settings['detuning']/1e6)
        ...
        for pind in range(pump_power_points):
            ...
            for find in range(cav_freq_points):
                ...
                # powerdat_full[pind, find] = np.sqrt(I_sig**2 + Q_sig**2)
            ...
            full_data['yaxis'] = pump_powers[0:pind+1] - exp_globals['Qbit_Attenuation']
            full_data['mags'] = diff_full[0:pind+1]
        ...
        userfuncs.SaveFull(saveDir, filename, ['full_data', 'full_data_off', 'voltages', 'current_voltage'], 

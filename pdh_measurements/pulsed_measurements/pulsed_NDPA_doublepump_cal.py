...
def get_default_settings():
    ...
    settings['meas_type'] = 'pulsed_NDPA_doublepump_cal'
    ...
    settings['pump_detuning']  = 20e6

    
    # cavity parameters
    settings['cav_power']               = -60
    settings['start_carr_freq']         = 5e9
    settings['stop_carr_freq']          = 7e9
    settings['carr_freq_points']        = 1e3

    settings['start_cav_detuning']      = 20e6
    settings['stop_cav_detuning']       = 30e6
    settings['cav_detuning_points']     = 10
...
def pulsed_NDPA_doublepump_cal(instruments, settings):
    ...
    filename_template = exp_settings['scanname'] + '_Δ{}MHz_{}V_{}dBm_' + stamp
    ...
    # Cavity settings
    carr_freq_points = exp_settings['carr_freq_points']
    carr_freqs = np.round(np.linspace(exp_settings['start_carr_freq'],exp_settings['stop_carr_freq'],carr_freq_points),-3)
    
    cav_detuning_points = exp_settings['cav_detuning_points']
    cav_detunings = np.round(np.linspace(exp_settings['start_cav_detuning'],exp_settings['stop_cav_detuning'],cav_detuning_points),-3)
    ...
    for i in range(pump_power_points):
        pump_amp_list[i] = get_amp_comps([pump_powers[i], -150, pump_powers[i]], total_pump_power)
    ...
    # Generator settings
    cavitygen.Freq   = carr_freqs[0] + cav_detunings[0]
    ...
    LO.Power = 12
    LO.Freq = carr_freqs[0] + cav_detunings[0] + exp_globals['IF']
    LO.Phase = 0
    LO.Output = 'On'
    LO.disable_pulse()
    ...
    # 3. for loop cav detuning
    # 4. for loop carr freq
    ###################################################
    for vind in range(voltage_points):
        ...
        for pind in range(pump_power_points):
            ...
            current_pump_power = pump_powers[pind] - exp_globals['Qbit_Attenuation']
            print(f'Current pump power: {current_pump_power}.')

            filename = filename_template.format(pump_detuning/1e6, current_voltage, current_pump_power)
            ...
            I_full = np.zeros((cav_detuning_points, carr_freq_points))
            Q_full = np.zeros((cav_detuning_points, carr_freq_points))
            powerdat_full = np.zeros((cav_detuning_points, carr_freq_points))
            phasedat_full = np.zeros((cav_detuning_points, carr_freq_points))
            diff_full = np.zeros((cav_detuning_points, carr_freq_points))

            I_off = np.zeros((cav_detuning_points, carr_freq_points))
            Q_off = np.zeros((cav_detuning_points, carr_freq_points))
            powerdat_off = np.zeros((cav_detuning_points, carr_freq_points))
            phasedat_off = np.zeros((cav_detuning_points, carr_freq_points))
            ...
            pumpgen.Output='Off'
            time.sleep(0.1)
            for cavfind in range(cav_detuning_points):
                for carrfind in range(carr_freq_points):
                    cavitygen.Freq = carr_freqs[carrfind] + cav_detunings[cavfind]
                    LO.Freq = carr_freqs[carrfind] + cav_detunings[cavfind] + exp_globals['IF']
                    cavitygen.Phase = 0
                    pumpgen.Phase = 0
                    LO.Phase = 0
                    ...
                    I_off[cavfind, carrfind] = I_sig
                    Q_off[cavfind, carrfind] = Q_sig
                    powerdat_off[cavfind, carrfind] = V_to_dBm(np.sqrt(I_sig**2 + Q_sig**2))
                    # powerdat_off[cavfind, carrfind] = np.sqrt(I_sig**2 + Q_sig**2)
                    phasedat_off[cavfind, carrfind] = np.arctan2(Q_sig,I_sig)*180/np.pi
                ...
                full_data_off = {}
                full_data_off['xaxis'] = carr_freqs/1e9
                full_data_off['mags'] = powerdat_off[0:cavfind+1]
                full_data_off['phases'] = phasedat_off[0:cavfind+1]
                full_data_off['I'] = I_off
                full_data_off['Q'] = Q_off

                single_data_off = {}
                single_data_off['xaxis'] = carr_freqs/1e9
                single_data_off['mag'] = powerdat_off[cavfind, :]
                single_data_off['phase'] = phasedat_off[cavfind, :]

                yaxis = cav_detunings[0:cavfind+1]/1e6
                labels = ['Carrier Freq (GHz)', 'Cav Detuning (MHz)']
            ...
            for cavfind in range(cav_detuning_points):
                for carrfind in range(carr_freq_points):
                    cavitygen.Freq = carr_freqs[carrfind] + cav_detunings[cavfind]
                    pumpgen.Freq = carr_freqs[carrfind]
                    LO.Freq = carr_freqs[carrfind] + cav_detunings[cavfind] + exp_globals['IF']
                    cavitygen.Phase = 0
                    pumpgen.Phase = 0
                    LO.Phase = 0
                    ...
                    I_full[cavfind, carrfind] = I_sig
                    Q_full[cavfind, carrfind] = Q_sig
                    powerdat_full[cavfind, carrfind] = V_to_dBm(np.sqrt(I_sig**2 + Q_sig**2))
                    # powerdat_full[cavfind, carrfind] = np.sqrt(I_sig**2 + Q_sig**2)
                    phasedat_full[cavfind, carrfind] = np.arctan2(Q_sig, I_sig)*180/np.pi

                    if first_it:
                        first_it = False
                
                diff_full[cavfind] = powerdat_full[cavfind] - powerdat_off[cavfind]
    

                ## Save Full data
                full_data = {}
                full_data['xaxis'] = carr_freqs/1e9
                full_data['cav_detunings'] = cav_detunings/1e6
                full_data['mags'] = diff_full[0:cavfind+1] #powerdat_full[0:cavfind+1]
                full_data['phases'] = phasedat_full[0:cavfind+1]
                full_data['abs_power'] = powerdat_full[0:cavfind+1]
                full_data['I'] = I_full[0:cavfind+1]
                full_data['Q'] = Q_full[0:cavfind+1]

                single_data = {}
                single_data['xaxis'] = carr_freqs/1e9
                single_data['mag'] = powerdat_full[cavfind, :]
                single_data['phase'] = phasedat_full[cavfind, :]

                yaxis = cav_detunings[0:cavfind+1]/1e6
                labels = ['Carrier/Pump Freq (GHz)', 'Cav Detuning (MHz)']
                simplescan_plot(full_data, single_data, yaxis, filename, labels, identifier='', fig_num=1)
                plt.savefig(os.path.join(saveDir, filename+'.png'), dpi = 150)
            
            userfuncs.SaveFull(saveDir, filename, ['full_data', 'full_data_off', 'voltages', 'pump_powers', 'current_voltage', 'current_pump_power', 'pump_detuning'], 

...
def get_default_settings():
    ...
    settings['meas_type'] = 'pulsed_NDPA_doublepump_powerscan'
    ...
    settings['start_pump_detuning']  = 10e6 #Delta
    settings['stop_pump_detuning']   = 50e6
    settings['pump_detuning_points'] = 2    
...
def pulsed_NDPA_doublepump_powerscan(instruments, settings):
    ...
    # Pump settings
    pump_detuning_points = exp_settings['pump_detuning_points']
    pump_detunings = np.round(np.linspace(exp_settings['start_pump_detuning'],exp_settings['stop_pump_detuning'],pump_detuning_points),-3)
    ...
    for i in range(pump_power_points):
        ...
        if exp_settings['lower_pump']:
            lsb_power = pump_powers[i]
        else:
            lsb_power = -150  
        pump_amp_list[i] = get_amp_comps([lsb_power, -150, usb_power], total_pump_power)
    ...
    for vind in range(voltage_points):
        ...
        I_full = np.zeros((pump_power_points, pump_detuning_points))
        Q_full = np.zeros((pump_power_points, pump_detuning_points))
        powerdat_full = np.zeros((pump_power_points, pump_detuning_points))
        phasedat_full = np.zeros((pump_power_points, pump_detuning_points))
        diff_full = np.zeros((pump_power_points, pump_detuning_points))
        ...
        for pind in range(pump_power_points):
            ...
            for find in range(pump_detuning_points):
                ...
                diff_full[pind, find] = powerdat_full[pind, find] - powerdat_off[0]
            ...
            ## Save Full data
            full_data = {}
            full_data['xaxis'] = pump_detunings/1e6
            full_data['yaxis'] = pump_powers[0:pind+1] - exp_globals['Qbit_Attenuation']
            full_data['mags'] = diff_full[0:pind+1] #powerdat_full[0:pind+1]

...
def get_default_settings():
    ...
    settings['meas_type'] = 'PDH_TripleDDC_manualDelays'
...
def pulsed_pdh_TripleDDC_manualDelays(instruments, settings):
    ##Instruments used
    ...
    configure_card_manualDelay(card, settings) #with variable delay
    ...
    for pind in range(power_points):
        ...
        ## trying to change hdawg reference clock source here
        ...
        #rescale to fractional amplitude for the plots.
            ...
            ax.scatter(pdhI_g[0, 0, :], pdhQ_g[0, 0, :], s=4, label='G')
            ax.legend()
            ax.set_xlabel('I')
            ax.set_ylabel('Q')
            ...
            ax.scatter(pdhI_e[0, 0, :], pdhQ_e[0, 0, :], s=4, label='E')
            ax.legend()
            ax.set_xlabel('I')
            ax.set_ylabel('Q')
            ax.axhline(y=0, color="black", linestyle="-")
            ...
            I_combined = np.concatenate((pdhI_g[0, 0, :], pdhI_e[0, 0, :]))
            Q_combined = np.concatenate((pdhQ_g[0, 0, :], pdhQ_e[0, 0, :]))
            ...
            ax.scatter(pdhI_g[0, 0, :], pdhQ_g[0, 0, :], s=4, label='PDH G')
            ax.scatter(pdhI_e[0, 0, :], pdhQ_e[0, 0, :], s=4, label='PDH E')
            ax.legend()
            ax.set_xlabel('I')
            ax.set_ylabel('Q')
            ax.axhline(y=0, color="black", linestyle="-")
            ...
            # ax = plt.subplot(1, 2, 1)
            # ax.scatter(pdhI_g[0, 0, :], s=4, label = 'G')
            # ax.scatter(pdhI_e[0, 0, :], s=4, label = 'E')
            # # plt.xlabel('I')
            # # plt.ylabel('Q')
            # plt.title('I data')
            # plt.legend()
            ...
            # ax = plt.subplot(1, 2, 2)
            # ax.scatter(pdhQ_g[0, 0, :], s=4, label = 'G')
            # ax.scatter(pdhQ_e[0, 0, :], s=4, label = 'E')
            # # plt.xlabel('I')
            # # plt.ylabel('Q')
            # plt.title('Q data')
            # plt.legend()

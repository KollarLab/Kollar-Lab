...
from utility.measurement_helpers import configure_card, configure_hdawg, estimate_time, read_and_process_TripleDDC, configure_card_manualDelay
...
def get_default_settings():
    ...
    settings['meas_type'] = 'PDH_TripleDDC_manualDelays_sweep'
...
def pulsed_pdh_TripleDDC_manualDelays_sweep(instruments, settings):
    ##Instruments used
    ...
    #configure_card(card, settings) #normal way
    # configure_card_manualDelay(card, settings) #with variable delay #### WILL NEED TO MOVE THIS INSIDE THE FREQ SWEEP
    ...
    new_combo_phase_g = np.zeros((power_points, len(freqs), segments))
    new_combo_phase_e = np.zeros((power_points, len(freqs), segments))
    
    # # # # 
    pdhI_g = np.zeros((power_points, len(freqs), segments))
    pdhQ_g = np.zeros((power_points, len(freqs), segments))
    pdhI_e = np.zeros((power_points, len(freqs), segments))
    pdhQ_e = np.zeros((power_points, len(freqs), segments))
    # # # #
    # PDH phase and power
    pdhPhase_g = np.zeros((power_points, len(freqs), segments))
    pdhPower_g = np.zeros((power_points, len(freqs), segments))
    pdhPhase_e = np.zeros((power_points, len(freqs), segments))
    pdhPower_e = np.zeros((power_points, len(freqs), segments))
    ...
    card_delay_bool = exp_settings['card_delay_bool']
    hdawg_delay_bool = exp_settings['hdawg_delay_bool']
    rounds = 8  ## using this to hard code the max card delay to be 32ns * 8
    # create delay arrays for the card and the AWG
    if card_delay_bool == True:
        card_delay_array = np.arange(0, rounds,1)*32e-9
    else:
        card_delay_array = np.zeros(rounds)

    if hdawg_delay_bool == True:
        hdawg_delay_array = np.linspace(0, -33e-9, rounds)
    else:
        hdawg_delay_array = np.zeros(rounds)

    random_card_delay = np.zeros((power_points, len(freqs), segments))
    random_hdawg_delay = np.zeros((power_points, len(freqs), segments))
    ...
    # tstart = time.time()
    first_it = True
    print(f'Current Powers| carrier:{carrier_power}, usb_sideband:{usb_power}, lsb_sideband:{lsb_power}, SGS:{gen_power}')
    for find in range(0, len(freqs)):
        # get random card and awg delay
        card_randint = random.randrange(rounds)
        card_delay = card_delay_array[card_randint]
        # print(random_card_delay.shape)
        # print(card_delay)
        random_card_delay[0, find, 0] = card_delay 

        hdawg_randint = random.randrange(rounds)
        hdawg_delay = hdawg_delay_array[hdawg_randint]
        random_hdawg_delay[0, find, 0] = hdawg_delay 

        manual_delays = {}
        manual_delays['card_delay'] = card_delay
        manual_delays['hdawg_delay'] = hdawg_delay
        exp_globals['manual_delays'] = manual_delays

        settings['exp_globals'] = exp_globals


        configure_card_manualDelay(card, settings)

        # Load AWG for cavity and qubit
        #position = start_time  #normal AWG timing
        position = start_time + exp_globals['manual_delays']['hdawg_delay']  #manusally messing with the AWG timing
        ...
        a_1 = np.unwrap((phasedat_carr_g[pind, find, :] - phasedat_usb_g[pind, find, :]), period=360)
        a_2 = np.unwrap((phasedat_carr_g[pind, find, :] - phasedat_lsb_g[pind, find, :]), period=360)
        new_combo_phase_g[pind, find, :] = a_1 + a_2

        # b_1 = np.unwrap((phasedat_carr_e[pind, find, :] - phasedat_usb_e[pind, find, :]), period=360)
        # b_2 = np.unwrap((phasedat_carr_e[pind, find, :] - phasedat_lsb_e[pind, find, :]), period=360)
        # new_combo_phase_e[pind, find, :] = b_1 + b_2
        ...
        # get new phase combo phase g and e
    ...
    phasedat['new_combo_g'] = new_combo_phase_g
    phasedat['new_combo_e'] = new_combo_phase_e

    selected_delays = {}
    selected_delays['random_card_delays'] = random_card_delay
    selected_delays['random_hdawg_delays'] = random_hdawg_delay
        
    full_data = {}
    full_data['xaxis']  = freqs/1e9
    full_data['phasedat'] = phasedat
    full_data['powerdat'] = powerdat
    full_data['selected_delays'] = selected_delays
    ...
    # # # # Plot PDH like data and the new combo
    fig = plt.figure(122)
    plt.clf()
    ...
    plt.plot(freqs/1e9, new_combo_phase_g[-1, :, 0], label='new_combo_g')
    plt.plot(freqs/1e9, new_combo_phase_e[-1, :, 0], label='new_combo_e')
    plt.xlabel('Freq (GHz)')
    ax.legend()
    ...
    plt.plot(freqs/1e9, powerdat_carr_g[0, :, 0].flatten(), label = 'g')
    plt.plot(freqs/1e9, powerdat_carr_e[0, :, 0].flatten(), label = 'e')
    plt.xlabel('Freq (GHz)')
    plt.grid()
    plt.legend()
    fig.tight_layout()

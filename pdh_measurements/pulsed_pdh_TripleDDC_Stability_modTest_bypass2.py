...
####import custom read and process functions for stability testing
from pdh_measurements.CustomMeasurementHelpers import*
...
def pulsed_pdh_TripleDDC_stability_modTest_bypass2(instruments, settings):
    
    '''Hacking to use the old qubit gen as the LO and not go through the fridge.
    Do not try to revert this script. Go back to the original. We messedw ith this too much.
    
    AK 7-16-25
    
    
    Setting up to do both regular pulsed spec analysis and triple DDC analysis on the dame acquisition run.
    '''
    ##Instruments used
    ...
    cavitygen.phase = 0
    cavitygen.enable_IQ()  ################################# CHANGE BACK TO ENABLE_IQ
    ...
    #####
    #data storage for triple DDC style analysis.
    #####
    # data saving arrays
    ...
    # # # # 
    Idat_g = np.zeros((stab_points, len(freqs), segments))
    Qdat_g = np.zeros((stab_points, len(freqs), segments))
    Idat_e = np.zeros((stab_points, len(freqs), segments))
    Qdat_e = np.zeros((stab_points, len(freqs), segments))
    
    
    #carrir alone
    carrierI = np.zeros((stab_points, len(freqs)))
    carrierQ = np.zeros((stab_points, len(freqs)))
    ...
    #spec storage arrays
    ###################
    powerdat = np.zeros((stab_points, len(freqs)))
    phasedat = np.zeros((stab_points, len(freqs)))
    specI = np.zeros((stab_points, len(freqs)))
    specQ = np.zeros((stab_points, len(freqs)))
    ###################
    ...
    ##create the digital down conversion filter if needed. Triple DDC version
    if exp_globals['IF'] != 0:
    ...
    ##create the digital down conversion filter if needed. SPEC version. So that we have the RF LO with the normal name
    if exp_globals['IF'] != 0:
        # #create Chebychev type II digital filter
        # filter_N = exp_globals['ddc_config']['order']
        # filter_rs = exp_globals['ddc_config']['stop_atten']
    ...
    for pind in range(stab_points):
        ###this is not a loop over reteptitions in lab time, and not in power
        ###it may be problematic that the freuquency loop is inside of this one.
    
        ###commented out to not touch generator power
        #cavitygen.power = gen_power #carrier_power + CAV_Attenuation #gen_power CHANGE BACK TO GEN_POWER

        
        ######################
        #triple DDC storage arrays
        ###################
        ...
        # hdawg.settings['System']['referenceClock'] = 'Internal'
        
        # time.sleep(3)
        
        # # LO.Ref.Source = 'EXT'
        # # cavitygen.Ref.Source = 'EXT'
        # hdawg.settings['System']['referenceClock'] = 'External'
        # ###############################
        ...
        if pind == 0:
            #####putting sstuff in special statement so they are done only once, not over and over versus time
            ########################################################
            cavitygen.power = gen_power # gen_power #carrier_power + CAV_Attenuation #gen_power CHANGE BACK TO GEN_POWER

            freq = freqs[0] #freqs[find]
            cavitygen.freq = freq
            cavitygen.phase = 0
            cavitygen.output = 'On'
        ...
        for find in range(0, len(freqs)):
            ...
            ###temporary hack. Will work for ONLY one frequency!!
            # freq = freqs[find]
            # cavitygen.freq = freq
            # cavitygen.phase = 0
            # cavitygen.output = 'On'
            ...
            #####original g-state read
            # carr_data_g, usb_data_g, lsb_data_g, xaxis_g = read_and_process_TripleDDC(card, settings, 
            #                                                              plot=first_it, 
            #                                                              IQstorage = True)
            
            #splitting read and process steps so that we can do spec processing on the triple DDC data 
            I_temp, Q_temp, Ip_triple_temp, Qp_triple_temp, Ip_spec_temp, Qp_spec_temp = read_only_both(card, settings, plot=first_it, IQstorage = True)
            #triple DDC
            carr_data_g, usb_data_g, lsb_data_g, xaxis_g = and_process_TripleDDC(I_temp, Q_temp, Ip_triple_temp, Qp_triple_temp,
                                                                                                           card, settings, 
                                                                                                             plot=first_it, 
                                                                                                             IQstorage = True)
            #pulse spec
            I_window, Q_window, I_full, Q_full, xaxis = and_process(I_temp, Q_temp, Ip_spec_temp, Qp_spec_temp,
                          card, settings, plot=first_it, IQstorage = True)
            ...
            pdhPhase_g[pind, find, :] = np.arctan2(Qdat_g[pind, find, :], Idat_g[pind, find, :]) * 180 / np.pi
            
            #store original I and Q of the carrier
            carrierI[pind, find] = I_final_carr_g
            carrierQ[pind, find] = Q_final_carr_g
            
            
            ####################
            #spec analysis and storage
            ##############
            ...
            powerdat[pind, find] = np.sqrt(I_final**2 + Q_final**2)
            phasedat[pind, find] = np.arctan2(Q_final, I_final)*180/np.pi #theta_sig-theta_back uncomment this for the relative angle
            
            specI[pind, find] = I_final
            specQ[pind, find] = Q_final
            ...
            plt.title('Tripple DDC')
            plt.ylabel('Phase')
            plt.xlabel('Run Number')
            
            ax2 = plt.subplot(1,2,2)
            ax2.clear()
            plt.scatter(nums, phasedat[0:(pind+1), find], label = 'spec,g', s = 25, color = 'mediumblue')
            ...
            plt.savefig(os.path.join(saveDir, filename+'_CarrierPhaseComparison.png'), dpi = 150)
    ...
    comparisondat = {}
    comparisondat['spec_powerdat'] = powerdat
    comparisondat['spec_phasedat'] = phasedat
    comparisondat['spec_I'] = specI
    comparisondat['spec_Q'] = specQ
    comparisondat['3DDC_powerdat'] = powerdat_carr_g
    comparisondat['3DDC_phasedat'] = phasedat_carr_g
    comparisondat['3DDC_I'] = carrierI
    comparisondat['3DDC_Q'] = carrierQ
    ...
    if freq_points > 1:
        ...
        I_data_g = Idat_g[:, 0, 0].flatten()
        Q_data_g = Qdat_g[:, 0, 0].flatten()
        phase_data_g = np.arctan2(Q_data_g, I_data_g)*180/np.pi

        I_data_e = Idat_e[:, 0, 0].flatten()
        Q_data_e = Qdat_e[:, 0, 0].flatten()
    ...
    t2 = time.time()
    # userfuncs.SaveFull(saveDir, filename, ['stab_points','freqs','xaxis','full_data', 'IQdat', 'full_time_g', 'single_time_g', 'full_time_e', 'single_time_e'],
    #                             locals(), expsettings=settings, instruments=instruments, saveHWsettings=first_it)
    userfuncs.SaveFull(saveDir, filename, ['stab_points','freqs','xaxis','full_data', 'IQdat','comparisondat'],

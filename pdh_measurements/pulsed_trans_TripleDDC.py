...
def get_default_settings():
    ...
    settings['meas_type'] = 'PulsedTrans_TrippleDDC'
...
def pulsed_trans_TripleDDC(instruments, settings):
    ##Instruments used
    ...
    for pind in range(power_points):
        ...
        print(f'Current Powers| carrier:{carrier_power}, sideband:{lsb_power}, SGS:{gen_power}')
        ...
        yaxis = {}
        yaxis['carr'] = carrier_powers[0:pind+1] #- CAV_Attenuation
        yaxis['usb'] = usb_powers[0:pind+1] #- CAV_Attenuation
        yaxis['lsb'] = lsb_powers[0:pind+1] #- CAV_Attenuation
        ...
        plot_data_e['mags_carr']   = np.transpose(np.transpose(powerdat_carr_e[0:pind+1])/drive_amps_lin_carr[0:pind+1])
        plot_data_e['mags_usb']   = np.transpose(np.transpose(powerdat_usb_e[0:pind+1])/drive_amps_lin_usb[0:pind+1])
        plot_data_e['mags_lsb']   = np.transpose(np.transpose(powerdat_lsb_e[0:pind+1])/drive_amps_lin_lsb[0:pind+1])
        plot_data_e['phases_carr'] = phasedat_carr_e[0:pind+1]
        plot_data_e['phases_usb'] = phasedat_usb_e[0:pind+1]
        plot_data_e['phases_lsb'] = phasedat_lsb_e[0:pind+1]

        single_data_e = {}
        single_data_e['xaxis'] = freqs/1e9
        single_data_e['mags_carr']   = powerdat_carr_e[pind]
        single_data_e['mags_usb']   = powerdat_usb_e[pind]
        single_data_e['mags_lsb']   = powerdat_lsb_e[pind]
        single_data_e['phases_carr'] = phasedat_carr_e[pind]
        single_data_e['phases_usb'] = phasedat_usb_e[pind]
        single_data_e['phases_lsb'] = phasedat_lsb_e[pind]
        ...
        userfuncs.SaveFull(saveDir, filename, ['power_points','freqs','xaxis','full_data', 'full_time_g', 'single_time_g', 'full_time_e', 'single_time_e'],

...
def read_and_process_JPA(card, settings, plot, IQstorage = True):
    '''
    Same read_and_process function, but for JPA.
    Without knowing the exact reason, original read_and_process function is giving 1/10 voltages when using np.mean
    '''
    card.ArmAndWait()
    raw_data = card.ReadAllData()
    Ip = raw_data[0][0]
    Qp = raw_data[1][0]
    ...
    #        plt.title('(hopefully) corrected phase angle')
        ...
        if settings['exp_globals']['IF'] == 0:
            ...
            # 2026/07/30 Won) There is the internal card driver problem that swaps ch1 and ch2. Ex) I=>ch1 / Q=>ch2 config will give raw_data[0][0]=Q and raw_data[1][0]=I
            # Now we are using I=>ch1 / Q=>ch2 and rotation angle = pi/2
            # Theoretically, I=>ch1 / Q=>ch2 should be using -pi/2. But internal swapping is making issue.

            # #Rotate by -pi/2; I=>ch2, Q=>ch1
            # Qprime_cos = Q_sin
            # Qprime_sin = -Q_cos
...
def V_to_dBm(voltage):
    # input in units of V, in order to match 1W = 30dBm
    # 10*np.log10(voltage**2) + 30
    return 20*np.log10(voltage) - 10*np.log10(50) + 30

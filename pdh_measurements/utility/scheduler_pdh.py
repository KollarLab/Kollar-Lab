...
def modulating_IQ(amp_list, phase_list, modulation_freq, time):
    sidebandM_amp, carrier_amp, sidebandP_amp = amp_list
    sidebandM_phase, carrier_phase, sidebandP_phase = phase_list
    sample_rate=2.4e9
    total_time = np.linspace(0, time, int(time*sample_rate))
    I = carrier_amp * np.cos(carrier_phase) + sidebandP_amp * np.cos(2*np.pi*modulation_freq*total_time + sidebandP_phase) + sidebandM_amp * np.cos(2*np.pi*modulation_freq*total_time - sidebandM_phase)
    Q = -carrier_amp * np.sin(carrier_phase) - sidebandP_amp * np.sin(2*np.pi*modulation_freq*total_time + sidebandP_phase) + sidebandM_amp * np.sin(2*np.pi*modulation_freq*total_time - sidebandM_phase)
     
    return I, Q

def modulating_I(amp_list, phase_list, modulation_freq, time):
    sidebandM_amp, carrier_amp, sidebandP_amp = amp_list
    # Normalize the amps (remove: 1/30/2025)
    ...
    # Use the new amp list func : get_amp_comp which uses amp_list directly and multiply (1/30/2025)
...
def modulating_Q(amp_list, phase_list, modulation_freq, time):
    sidebandM_amp, carrier_amp, sidebandP_amp = amp_list
    # Normalize the amps (remove: 1/30/2025)
    # sidebandM_amp, carrier_amp, sidebandP_amp = np.array([sidebandM_amp, carrier_amp, sidebandP_amp])/(sidebandM_amp + carrier_amp + sidebandP_amp)
    ...
    sample_rate=2.4e9
    total_time = np.linspace(0, time, int(time*sample_rate))
    # 2025-04-01 Q(t) sign error detected. Signs are flipped.
    Q = carrier_amp * np.sin(carrier_phase) + sidebandP_amp * np.sin(2*np.pi*modulation_freq*total_time + sidebandP_phase) - sidebandM_amp * np.sin(2*np.pi*modulation_freq*total_time - sidebandM_phase)
     
    return Q

def DPA_I(sample_rate, amp_list, freq_list, phase_list, pump_win, cav_win):
    # sample_rate=2.4e9
    sidebandM_amp, carrier_amp, sidebandP_amp = amp_list
    sidebandM_freq, carrier_freq, sidebandP_freq = freq_list
    sidebandM_phase, carrier_phase, sidebandP_phase = np.array([phase_list[0], phase_list[1], phase_list[2]])*np.pi
    
    # total_time = np.linspace(0, pump_win+cav_win, int((pump_win+cav_win)*sample_rate))
    total_time = np.arange(0, pump_win + cav_win, 1/sample_rate)
    cav_pos = pump_win/2
    pump_I = sidebandP_amp * np.cos(2*np.pi*sidebandP_freq*total_time + sidebandP_phase) + sidebandM_amp * np.cos(2*np.pi*sidebandM_freq*total_time + sidebandM_phase)
    cav_I = carrier_amp * np.cos(2*np.pi*carrier_freq*total_time + carrier_phase) * np.heaviside(total_time-cav_pos, 1) * np.heaviside((cav_pos+cav_win)-total_time, 1)
    return pump_I + cav_I

def DPA_Q(sample_rate, amp_list, freq_list, phase_list, pump_win, cav_win):
    # sample_rate=2.4e9
    sidebandM_amp, carrier_amp, sidebandP_amp = amp_list
    sidebandM_freq, carrier_freq, sidebandP_freq = freq_list
    sidebandM_phase, carrier_phase, sidebandP_phase = np.array([phase_list[0], phase_list[1], phase_list[2]])*np.pi
    ...
    total_time = np.arange(0, pump_win + cav_win, 1/sample_rate)
    cav_pos = pump_win/2
    pump_Q = sidebandP_amp * np.sin(2*np.pi*sidebandP_freq*total_time + sidebandP_phase) + sidebandM_amp * np.sin(2*np.pi*sidebandM_freq*total_time + sidebandM_phase)
    cav_Q = carrier_amp * np.sin(2*np.pi*carrier_freq*total_time + carrier_phase) * np.heaviside(total_time-cav_pos, 1) * np.heaviside((cav_pos+cav_win)-total_time, 1)
    return pump_Q + cav_Q
...
class AnalogChannel():
    ...
    def add_pulse(self, type, position, amplitude=1, **keyargs):
        ...
        if type=='pdh_I':
            amp_list = keyargs['amp_list']
            phase_list = keyargs['phase_list']
            mod_freq = keyargs['mod_freq']
            time = keyargs['time']
            pulse = modulating_I(amp_list, phase_list, mod_freq, time)
            pos_index = int(position*self.sample_rate)
            self.wave_array[pos_index:pos_index+len(pulse)] = pulse
        if type=='pdh_Q':
            amp_list = keyargs['amp_list']
            ...
            mod_freq = keyargs['mod_freq']
            time = keyargs['time']
            pulse = modulating_Q(amp_list, phase_list, mod_freq, time)
            pos_index = int(position*self.sample_rate)
            self.wave_array[pos_index:pos_index+len(pulse)] = pulse
        if type=='DPA_I':
            amp_list = keyargs['amp_list']
            freq_list = keyargs['freq_list']
            phase_list = keyargs['phase_list']
            pump_win = keyargs['pump_win']
            cav_win = keyargs['cav_win']
            pulse = DPA_I(self.sample_rate, amp_list, freq_list, phase_list, pump_win, cav_win)
            pos_index = int(position*self.sample_rate)
            self.wave_array[pos_index:pos_index+len(pulse)] = pulse
        if type=='DPA_Q':
            amp_list = keyargs['amp_list']
            ...
            pump_win = keyargs['pump_win']
            cav_win = keyargs['cav_win']
            pulse = DPA_Q(self.sample_rate, amp_list, freq_list, phase_list, pump_win, cav_win)
            pos_index = int(position*self.sample_rate)
            self.wave_array[pos_index:pos_index+len(pulse)] = pulse
...
class DigitalChannel():
    ...
    def reset(self, polarity=None):
        ...
        if polarity=='Pos':
            ...
            self.marker_array = np.ones(self.samples)*self.ID
        self.window_list = []

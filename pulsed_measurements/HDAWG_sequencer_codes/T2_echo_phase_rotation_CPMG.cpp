...
const tau_samples = (tau/pi_count-qbitTime)*sampleRate/2;
//const tau_samples = (tau)*sampleRate/2;
wave T2;
wave zero_buffer = zeros(tau_samples);
wave blank_buffer = zeros(tau*sampleRate);
T2 = join(qbit_clean*piAmp/2, zero_buffer);

for (i=0; i<pi_count-1;i++){
  T2 = join(T2, qbit_clean*piAmp, zero_buffer, zero_buffer);
}
T2 = join(T2, qbit_clean*piAmp, zero_buffer, qbit_clean*piAmp/2);

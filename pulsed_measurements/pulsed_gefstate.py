...
#    qubit_marker.add_window(0,start_time+2e-6)
    ...
    first_it = True
    Iblob_cspec_e = np.zeros(attempted_averages)
    Qblob_cspec_e = np.zeros(attempted_averages)
    powerdat_cspec_e = np.zeros(attempted_averages)
    phasedat_cspec_e = np.zeros(attempted_averages)
    if first_it:
        tstart = time.time()
...
#                time.sleep(0.1)
    ...
    # Make FWHM gaussian circle, count by numbers of points in the gaussian circle. The normalized gaussian function contains erf(ln(2))~0.7609 points in the FWHM.
    r_cspec_e = find_pointsincircle(Iblob_cspec_e, Qblob_cspec_e, Iblob_cspec_e_avg, Qblob_cspec_e_avg, attempted_averages)
...
#    qubit_marker.add_window(0,start_time+2e-6)
    ...
    first_it = True
    Iblob_csspec_e = np.zeros(attempted_averages)
    Qblob_csspec_e = np.zeros(attempted_averages)
    powerdat_csspec_e = np.zeros(attempted_averages)
    phasedat_csspec_e = np.zeros(attempted_averages)
    if first_it:
        tstart = time.time()
...
#                time.sleep(0.1)
        ...
        # Store data
        Iblob_csspec_e[avgind] = I_final
        Qblob_csspec_e[avgind] = Q_final
        Iblob_csspec_e_avg = np.mean(Iblob_csspec_e)
        Qblob_csspec_e_avg = np.mean(Qblob_csspec_e)
        powerdat_csspec_e[avgind] = np.sqrt(I_final**2 + Q_final**2)
        phasedat_csspec_e[avgind] = np.arctan2(Q_final, I_final)*180/np.pi
    ...
    # Make FWHM gaussian circle, count by numbers of points in the gaussian circle. The normalized gaussian function contains erf(ln(2))~0.7609 points in the FWHM.
    r_csspec_e = find_pointsincircle(Iblob_csspec_e, Qblob_csspec_e, Iblob_csspec_e_avg, Qblob_csspec_e_avg, attempted_averages)
    ...
    ax.scatter(Iblob_cstrans, Qblob_cstrans, s=4, c='b', label='carrier and sideband_trans')
    ax.scatter(Iblob_csspec, Qblob_csspec, s=4, c='r', label='carrier and sideband_spec')
    ax.scatter(Iblob_csspec_e, Qblob_csspec_e, s=4, c='lawngreen', label='carrier and sideband_spec_estate')

...
def pulsed_gefstate_estate_SGS(instruments, settings):
...
#    configure_hdawg(hdawg, settings)
    ...
    for i in range(power_points):
        ...
        plt.title('Modulation Freq(MHz): {}, Single_shots: {}, Sideband drive power: {}'.format(exp_settings['mod_freq']/1e6, single_shots, np.round(dBm_list[i][0]-CAV_Attenuation,1)))
        plt.xlabel("I")
        plt.ylabel("Q")
        ...
        save_name = filename + '_fstate_' + 'SBpower: {}'.format(np.round(dBm_list[i][0]-CAV_Attenuation,1))

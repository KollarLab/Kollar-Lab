...
def get_default_settings():
    ...
    settings['meas_type'] = 'pulsed_measurement_window'
...
def pulsed_measwinshape(instruments, settings):
    ...
    T1 = exp_settings(['T1'])
    weighing = exp_settings(['weighing'])
...
#                time.sleep(0.1)
        ...
        # Weighing the measurement window with exp decay in T1 time
        if weighing:
            t = np.linspace(0, T1, 2.4e9*T1)
            I_window = I_window * np.exp(-t/T1)
            Q_window = Q_window * np.exp(-t/T1)

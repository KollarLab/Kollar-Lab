...
def pulsed_powerscan_singleshot2(instruments, settings):
    ...
    for pind in range(power_points):
        ...
        if exp_settings['sidebandonoff'] == 'Off':
            ...
            LO.freq = CAV_freq_list[pind] - exp_globals['IF']
        cavitygen.Power  = SGS_power
        cavitygen.enable_pulse()
        cavitygen.enable_IQ()
        cavitygen.Output = 'On'
        ...
        cavity_I.add_pulse('pdh_I', position=cav_position,
                            amp_list = amp_list[pind], phase_list = phase_list,
                            mod_freq = mod_freq,
                            time = window_time)
...
#                time.sleep(0.1)
    ...
    arb = 100
    if exp_settings['sidebandonoff'] == 'On':
        title_mod = 'Modulation Freq(MHz): ' + str(exp_settings['mod_freq']/1e6) + ', '
        scan_letter = 'sideband'
    if exp_settings['sidebandonoff'] == 'Off':
        title_mod = ''
        scan_letter = 'carrier'
    ...
    for i in range(power_points):
        ...
        plt.clf()
        ax = plt.subplot(1,1,1)
        ax.scatter(Iblob_trans[i], Qblob_trans[i], s=4, c='k', label='meas')
        ax.scatter(Iblob_spec[i], Qblob_spec[i], s=4, c='r', label='pi pulse + meas')
        ...
        plt.title('{} Single_shots: {}, Scanning {} power: {}'.format(title_mod, segments, scan_letter, powers[i]))
        plt.xlabel("I")
        plt.ylabel("Q")
        plt.axhline(y = 0, color ="black", linestyle ="-")
        plt.axvline(x = 0, color ="black", linestyle ="-") 
        ...
        save_name = filename + scan_letter + '_power scan_' + str(powers[i])
        userfuncs.savefig(fig, save_name, saveDir, png=True)

...
def pulsed_spec_bypasstest2(instruments, settings):
...
#    qubit_marker.add_window(0,start_time+2e-6)
    ...
    ######################
    if exp_globals['IF'] != 0:
         mod_freq = 24e6  #!!!!!!!!!!@!!!!!!!!WARNING - Hard coded number. Change it at your own peril.
         theta_lo = 0
    ...
    specI = np.zeros((len(powers), len(freqs)))
    specQ = np.zeros((len(powers), len(freqs)))
    ###################
    ...
    for powerind in range(len(powers)):
        ...
        ####hard coded delay for stability stuff. Goal is to get a scan a minute
        time.sleep(50)
...
#            time.sleep(0.1)
            ...
            # #the original joint read and process function
...
#            Q_full_net = Q_full#full Q data without background subtracted
            ...
            ####restoring the spec I and Q.
            specI[powerind, find] = I_final
            specQ[powerind, find] = Q_final
            
            
            ################
            #triple ddc carrier analysis
            ##########
            ...
            powerdat_carr_g[powerind, find, :] = np.sqrt(I_final_carr_g**2 + Q_final_carr_g**2)
            phasedat_carr_g[powerind, find, :] = np.arctan2(Q_final_carr_g, I_final_carr_g)*180/np.pi
            
            #store original I and Q of the carrier
            carrierI[powerind, find] = I_final_carr_g
            carrierQ[powerind, find] = Q_final_carr_g
            ################
            
        
        ####phase plot once for every main loop (fake power)    
        fig = plt.figure(445, constrained_layout = True)
        if powerind ==0:
            plt.clf()
            
        nums = np.arange(0,powerind+1,1)
        
        ax1 = plt.subplot(1,2,1)
        ax1.clear()
        plt.scatter(nums, phasedat_carr_g[0:(powerind+1), 0,0], label = '3DDC carrier,g', s = 25)
        # plt.scatter(nums, phasedat_lsb_g[0:(powerind+1), 0,0], label = 'lsb,g', s = 25)
        ...
        ax2.clear()
        plt.scatter(nums, phasedat[0:(powerind+1), 0], label = 'spec,g', s = 25, color = 'mediumblue')
        ...
        #############
        #save the triple DDC results and spec results together
        #############
        comparisondat = {}
    ...
    userfuncs.SaveFull(saveDir, filename, ['powers','freqs', 'xaxis',
                                                       'powerdat', 'phasedat',
                                                       'full_data', 'single_data', 'comparisondat'], 
                                                       #'full_time', 'single_time'],
                                                     locals(), 
                                                     expsettings=settings, 

...
def Rabi_sweep(soc,soccfg,instruments,settings):
    ...
    config = {
        ...
        'qub_freq'        : exp_settings['qub_freq']/1e6, #Placeholder
    ...
    amp_int = np.zeros(len(gpts))
    ang_int = np.zeros(len(gpts))
    amps    = np.zeros((len(gpts),total_samples))
    angles  = np.zeros((len(gpts),total_samples))
    ...
    for g in range(0,len(gpts)):
        ...
        amps[g] = np.sqrt(I_full**2+Q_full**2)
        angles[g] = np.arctan2(Q_full,I_full)*180/np.pi
        amp_int[g] = np.sqrt(I_final**2+Q_final**2)
        ang_int[g] = np.arctan2(Q_final, I_final)*180/np.pi
        ...
        plt.clf()
        plt.subplot(121)
        plt.plot(gpts, amp_int)
        plt.xlabel('Gain (a.u.)')
        plt.ylabel('Amplitude')  
        plt.subplot(122)
        plt.plot(gpts, ang_int)
        plt.xlabel('Gain (a.u.)')
        plt.ylabel('Phase')  
        ...
        general_colormap_subplot(ax, xaxis, gpts, amps, ['Time (us)', 'Gain (a.u.)'], 'Raw data\n'+filename)
    
        plt.savefig(os.path.join(saveDir, filename+'_fulldata.png'), dpi = 150)
        userfuncs.SaveFull(saveDir, filename, ['gpts','xaxis', 'amps', 'amp_int'], locals(), expsettings=settings, instruments=instruments)

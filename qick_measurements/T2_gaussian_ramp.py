...
class T2_sequence(AveragerProgram):
    def initialize(self):
        ...
        self.add_gauss(ch=gen_ch, name="ex", sigma=sigma,length=int(sigma*num_sigma),maxv=int(self.soccfg['gens'][qub_ch]['maxv']/2))

        self.set_pulse_registers(ch=gen_ch, style="flat_top", waveform='ex',length=self.us2cycles(self.cfg["meas_window"],gen_ch=gen_ch))

...
def pulsed_trans(soc, soccfg, instruments, settings):
    ...
    for a in range(len(apts)):
        ...
        for f in range(len(fpts)):
            ...
            if exp_settings.get('filter', False):
                soc.rfb_set_gen_filter(
                    config['cav_channel'], fc=board_freq / 1000,
                    ftype='bandpass', bw=exp_globals['cav_channel']['BW']
                )
                soc.rfb_set_ro_filter(
                    config['ro_channel'], fc=board_freq / 1000,
                    ftype='bandpass', bw=exp_globals['ro_channel']['BW']
                )

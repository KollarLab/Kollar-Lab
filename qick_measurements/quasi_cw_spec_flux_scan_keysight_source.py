...
def get_spec_flux_scan_settings():
    ...
    settings['qub_gain']     =  1000#1000 #30000
    settings['quasi_CW_len'] = 30 #us
    settings['freq_start']      = 4e9
    settings['freq_stop']       = 6e9
    settings['freq_points']     = 3
...
def spec_flux_scan(soc,soccfg,instruments,settings):
    ...
    skip_autoscan = exp_settings['skip_autoscan']
    ...
    config = {
        ...
        'cav_freq'        : exp_settings['readout_freq']/1e6, # Going to sweep through this in for loop if choosing autoscan; otherwise will skip auto scan and readout only at this freq
    ...
    if skip_autoscan == True:
        projected_time = config['reps']*config['soft_avgs']*config['freq_points']*rep_period/1e6
        print("Projected Time for a single flux point: " + str(projected_time))
    ...
    trans_mags   = np.zeros((flux_pts, autoscan_set['freq_points']))
    trans_phases = np.zeros((flux_pts, autoscan_set['freq_points']))
    
    #Dummy arrays for spec scan
    mags   = np.zeros((flux_pts, exp_settings['freq_points']))
    phases = np.zeros((flux_pts, exp_settings['freq_points']))
    ...
    for vind in range(len(full_voltages)):
        ...
        if exp_settings['stability'] == True: # If running a stability scan, will ramp to starting voltage on the first iteration but then never ramp again.
            if first_it == True:
                Dual_gen.Ch1_dc_voltage_ramp(-np.round(full_voltages[0][0], 4)) #minus sign here is because FBL_1 has the opposite polarity with the magnet and FBL_2
                Dual_gen.Ch2_dc_voltage_ramp(np.round(full_voltages[0][1], 4))
                time.sleep(0.1)
                print('This is a stability scan. The fluxes will not change during the scan:')
                print('Voltage {}: initial {} V, final {} V'.format(str(1),-np.round(full_voltages[0][0], 4),-np.round(full_voltages[0][0], 4)))
                print('Voltage {}: initial {} V, final {} V'.format(str(2),np.round(full_voltages[0][1], 4),np.round(full_voltages[0][1], 4)))
...
# =============================================================================
        ...
        if skip_autoscan == False:
            ...
            config['cav_freq'] = exp_settings['readout_freq']/1e6
        ...
        trans_labels = ['Freq (GHz)','Flux ' + str(qubit_num)]
        spec_labels  = ['Freq (GHz)','Flux ' + str(qubit_num)]
    ...
    tfinal = time.time()
    print('Total elapsed time = ' + str(np.round((tfinal-tstart)/3600,1)) + ' hrs')

...
def configure_card_dual(card, card_config, settings):
    ...
    total_time = max([boost['length'], readout['length']])+abs(readout['position']-boost['position']) #2 is used for extracting back ground
    ...
    background_window = default_meas['background_window']
    card_time = total_time + emp_delay + init_buffer + post_buffer+background_window

    meas_samples = card_config['sampleRate']*card_time

    #Compute trigger delay, has to be multiple of 32ns for Acquiris
    trigger_delay = np.floor((start_time - init_buffer-background_window)/32e-9)*32e-9
...
def extract_data_heterodyne(raw_data, xaxis, settings, channel):
    ...
    background_window = settings['base']['background_window']
    
    timestep   = xaxis[1] - xaxis[0]
    window_width = int(meas_window*0.8/timestep)
    background_width = int(background_window/timestep)
    data_start = int((init_buffer + emp_delay + background_window+meas_window*0.2)/timestep)
    back_start = int((emp_delay + init_buffer)/timestep)

    
    data_x = xaxis[data_start:data_start+window_width]
    background = raw_data[back_start:back_start+background_width]
